    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-line bg-panel p-4 print:hidden">
        ...
        <p className="mb-4 text-xs text-ink-soft">
          Tip: use <strong>Print</strong> → save as PDF. Labels are sized so each sticker stays on one page (no cut-off
          QR codes).
        </p>
      ...
      <div className="print-area space-y-6 print:space-y-0">
        {selectedBags.map((bag, index) => (
          <BagLabelSection key={bag.id} bag={bag} content={content} isFirst={index === 0} />
        ))}
...
function BagLabelSection({
  ...
  return (
    <section className={`print-bag-section space-y-4 ${isFirst ? '' : 'print:break-before-page'}`}>
      ...
      {content !== 'meds' && (
        <div className="print-label print-bag-label rounded-xl border-2 border-dashed border-sea bg-panel p-4">
          <div className="mb-2 flex items-center gap-2">
            <img src="/doserx-logo.png" alt="" className="h-7 w-7 rounded-md object-cover" />
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-sea-mid">DoseRX · Bag shift QR</p>
              <p className="text-[10px] text-ink-soft">Unique to this bag — sign out / return</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="print-qr shrink-0">
              <QRCodeSVG value={bagPayload} size={120} level="M" includeMargin />
            </div>
            <div className="min-w-0">
              <p className="font-display text-2xl font-extrabold leading-none">{bag.code}</p>
              <p className="mt-1 text-sm text-ink-soft">{bag.name}</p>
              <p className="mt-1 text-xs text-ink-soft">
                {bag.type === 'controlled' ? 'Controlled drugs' : bag.grade} · Seal {bag.sealNumber}
              </p>
              <p className="mt-2 font-mono text-[9px] text-ink-soft/70">{bagPayload}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {content !== 'bags' && (
        <div className="print-label-grid grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {bag.items.map((item) => (
...
function MedLabelCard({ bag, item }: { bag: DrugBag; item: StockItem }) {
  ...
  return (
    <div className="print-label print-med-label rounded-xl border border-line bg-panel p-2.5 shadow-sm">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-1 rounded bg-sea/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-sea">
          <Pill size={9} /> Med QR
        </span>
        {item.controlled && (
          <span className="rounded bg-cd-soft px-1.5 py-0.5 text-[9px] font-bold text-cd">
            Sch {item.schedule}
          </span>
        )}
      </div>
      <div className="flex gap-2.5">
        <div className="print-qr shrink-0">
          <QRCodeSVG value={payload} size={96} level="H" includeMargin />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-bold leading-tight text-ink">{item.name}</p>
          <p className="text-[10px] leading-snug text-ink-soft">{item.presentation}</p>
          <p className="mt-1 font-mono text-[9px] font-bold leading-tight text-sea">{tracking}</p>
          <p className="mt-1 text-[10px] leading-snug">
            <span className="font-semibold">Bag</span> {bag.code}
          </p>
          <p className="text-[10px] leading-snug">
            <span className="font-semibold">Batch</span> {item.lotNumber} ·{' '}
            <span className="font-semibold">Exp</span> {format(new Date(item.expiryDate), 'MMM yyyy')}
          </p>
          <p className="text-[10px] leading-snug">
          ...
          <button
            ...
            className="mt-1.5 rounded border border-line bg-surface px-2 py-0.5 text-[10px] font-semibold print:hidden hover:border-sea"
            title={payload}
          >
            Copy QR payload
          </button>
      ...
      <p className="mt-1.5 hidden font-mono text-[8px] text-ink-soft/50 print:block print:truncate">{payload}</p>
      <p className="mt-1 flex items-start gap-1 text-[9px] text-ink-soft/70 print:hidden">
        <CheckSquare size={9} className="mt-0.5 shrink-0" />
